...
# This class overrides the training_step to solve the 256 vs 832 broadcast error
class NetworkAnomalySpecialist(LagLlamaLightningModule):
    def training_step(self, batch, batch_idx):
        # Compute loss but don't let it crash on broadcast
        # We manually slice the results to match the target batch size
        ...
        # SLICING LOGIC: We take the last 256 points of the model output
        # to match the context window you actually want to train on.
        target = past_target[:, -256:]

        # Calculate loss on the aligned slices
...
def run_finetuning():
    ...
    trainer = lp.Trainer(
        max_epochs=5, accelerator="gpu", devices=1,
        default_root_dir="network_finetuning_v57"
    )

    print("--- STARTING SPECIALIZATION LOOP (Slicing Enabled) ---")
