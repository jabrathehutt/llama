...
def calculate_detection_speed(anomaly_start_time, detection_timestamps, time_index, freq_minutes=5):
    ...
    # Ensure anomaly_start_time is a pandas timestamp
    if not isinstance(anomaly_start_time, pd.Timestamp):
        anomaly_start = pd.to_datetime(anomaly_start_time)
    else:
        anomaly_start = anomaly_start_time
    
    # Ensure all detection timestamps are pandas timestamps
    detection_timestamps = [pd.to_datetime(dt) if not isinstance(dt, pd.Timestamp) else dt 
        ...
        try:
            ...
            if flow_result:
                ...
                if speed_info['detected_periods'] > 0:
                    ...
                    if speed_info['total_anomaly_periods'] > 0:
                        print(f"  ✗ No anomalies detected ({speed_info['missed_periods']} missed)")
                    else:
                        print(f"  ○ No anomaly periods in this flow")
...
def analyze_single_flow_with_speed(df, series_id, predictor):
    ...
    for idx in analysis_indices:
        try:
            ...
            # Ensure current_time is a pandas Timestamp
            if isinstance(validation_timestamps[idx], np.datetime64):
                current_time = pd.Timestamp(validation_timestamps[idx])
            elif isinstance(validation_timestamps[idx], pd.Timestamp):
                current_time = validation_timestamps[idx]
            else:
                current_time = pd.to_datetime(validation_timestamps[idx])
            ...
            if is_anomaly:
                ...
                # FIXED: Ensure detection_time is calculated correctly with Timestamp
    ...
    # Add detection metrics to speed_metrics for easier analysis
    speed_metrics.update({
        'flow_key': series_id,
        'pattern': series_id.split('_')[-1] if '_' in series_id else 'unknown',
        'detection_count': len(detection_results),
        'true_positives': detection_metrics.get('true_positives', 0),
        'false_positives': detection_metrics.get('false_positives', 0),
        'true_negatives': detection_metrics.get('true_negatives', 0),
        'false_negatives': detection_metrics.get('false_negatives', 0),
        'precision': detection_metrics.get('precision', 0),
        'recall': detection_metrics.get('recall', 0),
        'f1_score': detection_metrics.get('f1_score', 0),
        'accuracy': detection_metrics.get('accuracy', 0),
        'mae': detection_metrics.get('mae', 0),
        'rmse': detection_metrics.get('rmse', 0),
    })
...
def calculate_detection_metrics(y_true, y_pred, predictions, actuals, prediction_intervals):
    ...
    if y_true is not None and len(y_true) > 0:
        try:
            ...
            y_pred_array = np.array(y_pred[:len(y_true_array)])
            
            metrics['precision'] = float(precision_score(y_true_array, y_pred_array, zero_division=0))
            metrics['recall'] = float(recall_score(y_true_array, y_pred_array, zero_division=0))
            metrics['f1_score'] = float(f1_score(y_true_array, y_pred_array, zero_division=0))
...
def generate_comprehensive_report(all_flow_results, all_speed_metrics, output_dir="lag_llama_speed_analysis"):
    ...
    with open(summary_path, 'w') as f:
        ...
        if not speed_metrics_df.empty:
            ...
            if 'precision' in speed_metrics_df.columns:
                f.write(f"  Average Precision: {speed_metrics_df['precision'].mean():.4f}\n")
                f.write(f"  Average Recall: {speed_metrics_df['recall'].mean():.4f}\n")
                f.write(f"  Average F1-Score: {speed_metrics_df['f1_score'].mean():.4f}\n")
                f.write(f"  Average Accuracy: {speed_metrics_df['accuracy'].mean():.4f}\n")
                f.write(f"  Average MAE: {speed_metrics_df['mae'].mean():.4f}\n")
                f.write(f"  Average RMSE: {speed_metrics_df['rmse'].mean():.4f}\n")
...
def main():
    ...
    if all_speed_metrics and len(all_speed_metrics) > 0:
        ...
        delays = [m.get('avg_detection_delay_sec', 0) for m in all_speed_metrics
                 if m.get('avg_detection_delay_sec') is not None and not pd.isna(m.get('avg_detection_delay_sec'))]
        ...
        if total_periods > 0:
            print(f"  Detection rate: {detected_periods/total_periods*100:.1f}%")
        else:
        ...
        # Performance metrics
        if 'precision' in all_speed_metrics[0]:
            precisions = [m.get('precision', 0) for m in all_speed_metrics]
            recalls = [m.get('recall', 0) for m in all_speed_metrics]
            f1_scores = [m.get('f1_score', 0) for m in all_speed_metrics]
