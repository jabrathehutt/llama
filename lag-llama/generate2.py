...
FREQUENCY = '5min'
OUTPUT_FILE = '/root/llama/lag-llama/network_metrics_data.csv'
FLOW_GROUP_FILE = 'simplified_flow_group_mapping.csv'
NUM_FLOWS_PER_GROUP = 334

# AS-Konstanten (Used for transit nodes)
AS_IDS = {
    'AS100': 100, 'AS200': 200, 'AS300': 300, 'AS400': 400
}


# --- 1. PATTERN GENERATORS (Deterministic) ---

def generate_linear(time_index, params):
    """Generates a pure linear pattern (no noise)."""
...
def generate_sinus(time_index, params):
    """Generates a pure additive sinusoidal pattern (no noise, no multiplicative growth)."""
    total_minutes = (time_index - time_index[0]).total_seconds() / 60
...
def generate_jumps(time_index, params):
    """Generates a linear pattern with predictable, deterministic jumps (no noise)."""
    total_minutes = (time_index - time_index[0]).total_seconds() / 60
...
master_templates = [
    ...
    # GRUPPE 2: SINUS FLOWS (Drift only)
    {
        ...
        'anomalies': [
            {'type': 'drift', 'start_time': '2025-01-07 12:00', 'duration_min': 240, 'magnitude_traffic': 15.0},
        ],
        'count': NUM_FLOWS_PER_GROUP
    },
...
def generate_full_master_dataset():
    ...
    for template in master_templates:
        ...
        for i in range(template['count']):
            ...
            params_copy['traffic_start'] = params_copy.get('traffic_start', 0.0) + (i * 0.01)  # Unique start offset

            flow_df = generate_data(time_index, {'pattern': template['pattern'], 'params': params_copy})
            flow_df['flow_key_id'] = flow_id
            flow_df['is_anomaly'] = False  # Initialize Ground Truth
            ...
            for anomaly in template['anomalies']:
                ...
                flow_df = add_anomaly(flow_df, anomaly_type=anomaly_type, **anomaly_copy)

            # --- Metadaten (Only necessary ASNs are included) ---
    ...
    # ⚠️ REMOVAL OF UNWANTED COLUMNS
    # The ingressRouter, egressRouter, and application columns are removed here
    final_df.drop(columns=['application'], errors='ignore', inplace=True)
    final_df.drop(columns=['ingressRouter', 'egressRouter'], errors='ignore', inplace=True)

    # Final Timestamp Conversion
    ...
    print(f"TOTAL FLOW KEYS: {len(final_df['flow_key_id'].unique())}")
    print(f"Total Ground Truth Anomalies: {final_df['is_anomaly'].sum()}")
    print(f"=========================================================")
    return final_df
    ...
    for i, flow_id in enumerate(sampled_flow_ids):
        ...
        if not anomalies.empty:
            ax.scatter(anomalies['timestamp'], anomalies['traffic_volume_Tbits'],
                       color='red', label='Anomalies (Spike/Drift)', s=40, zorder=5)
...
if __name__ == "__main__":
    ...
    # Startet die Visualisierung (muss nach der Generierung und Speicherung der CSVs erfolgen)
